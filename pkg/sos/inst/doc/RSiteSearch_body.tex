...
If either the \pkg{WriteXLS} package and compatible Perl code
are properly installed or if you are running Windows with
the \pkg{RODBC} package, this produces an Excel file in the
...
to the \pkg{fda} package and its companion books, which further
...
was \pkg{deSolve}, which provides, ``General solvers for initial
...
to the subject.  The second package was \pkg{PKfit}, which
...
\section{Discussion}
We have found {\tt findFn} in the \pkg{sos} package to
...
Users of \pkg{sos} may also wish to consult Crantastic
